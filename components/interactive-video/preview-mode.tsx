  ...
  if (showScore) {
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-6 py-12 px-4">
        ...
        {percentage === 100 && (
          <p className="text-lg font-medium text-success">Tebrikler! Mukemmel skor!</p>
        )}
      ...
      >
        ...
        {isYT ? (
          <div className="aspect-video w-full relative bg-black">
            <div ref={ytContainerRef} className="absolute inset-0 flex items-center justify-center" />
          ...
          <video
            ref={videoRef}
            src={videoSource.src}
            className={`w-full h-full bg-black ${isFullscreen ? "max-h-screen object-contain" : "aspect-video"}`}
            playsInline
            onClick={togglePlayPause}
                    ...
                    <div className="flex flex-col gap-3">
                      {answers[answers.length - 1]?.isCorrect ? (
                        <div className="flex items-center gap-2 rounded-lg bg-success/10 px-4 py-3">
                          ...
                          <span className="font-medium text-success">Dogru Cevap!</span>
                        </div>
                      ) : (
                        <div className="flex items-center gap-2 rounded-lg bg-destructive/10 px-4 py-3">
                          <XCircle className="h-5 w-5 text-destructive" />
                          <span className="font-medium text-destructive">Yanlis Cevap</span>
                        </div>
                      )}
                      {activeInteraction.explanation && (
